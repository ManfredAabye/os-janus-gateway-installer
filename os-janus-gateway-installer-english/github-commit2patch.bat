...
if not defined REPOSPEC (
    set /p "REPOSPEC=GitHub repo (owner/repo) or commit URL: "
)
...
if not exist "%OUTDIR%" (
    ...
    if errorlevel 1 (
        echo Error: could not create target directory: "%OUTDIR%"
        exit /b 1
    )
)
...
if /i "%MODE%"=="latest" (
    echo Resolving latest commit from %OWNER%/%REPO% on branch "%BRANCH%" ...
    ...
    echo Using commit from URL: %SHA%
)

if not defined SHA (
    echo Error: could not resolve commit SHA.
    echo Tip: check owner/repo, branch, or commit URL.
    exit /b 1
)
...
if errorlevel 1 (
    echo Error: could not download patch.
    exit /b 1
)
...
echo Please provide owner/repo or a GitHub commit URL,
echo e.g. meetecho/janus-gateway or https://github.com/owner/repo/commit/SHA
exit /b 1
